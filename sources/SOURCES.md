...
| 13 | Andersson, Gillis, Horn, Rawlings & Diehl, "CasADi – a software framework for nonlinear optimization and optimal control", *Math. Prog. Comp.* 11, 2019; Wächter & Biegler (IPOPT), *Math. Prog.* 106, 2006; Betts, *Practical Methods for Optimal Control … (Direct Collocation)*, 2010 | the **provably minimum-time** lap in `tracks/opt_lap.py` (`--line=ocp`): friction-circle point-mass minimum-time OCP in the arc-length domain, trapezoidal direct collocation, solved with CasADi + IPOPT to a KKT point (local optimum for the reduced model; the full tricycle then tracks the resulting line) |
